...
fg_plot <- function(
    ...
    summary_meta <- unlist(fg@summary_desc[[type]][
    ...
    if (type=="node") {
        ...
        if (!"NONE"%in%node_labels) {
            node_labels <-
                node_labels[node_labels%in%names(fg@feat[[type]])]
...
fg_plot_box <- function(
    ...
    if (all_labels) {
        id_ord <- order(fg@meta[,summary_meta["class"]])
...
fg_plot_box_set <- function(
    ...
    node_edge <- ifelse(is.character(node_edge),
                        which(fg@graph$v$phenotype==node_edge), node_edge)
...
fg_plot_pVSdiff <- function(
    ...
    dta$avg_count <- colMeans(as.matrix(fg@feat$node$count))
...
fg_save_plots <- function(
    ...
    for (type in plot_types) {
        if (is.null(fg@summary[[type]])) next
