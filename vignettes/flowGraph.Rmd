...
no_cores <- 1
data(fg_data_pos2)
fg <- flowGraph(fg_data_pos2$count, meta=fg_data_pos2$meta,
                prop=FALSE, specenr=FALSE,  no_cores=no_cores)
gr <- fg_get_graph(fg)
gr <- ggdf(gr)
...
fg <- flowGraph(
    ...
    class=fg_data_pos2$meta$class, # a vector indicating each sample's class
...
fg <- flowGraph(fg_data_pos2$count, class=fg_data_pos2$meta$class, 
                no_cores=no_cores)
# fg <- flowGraph(fg_data_pos2$count, meta=fg_data_pos2$meta, class="class"
#                 no_cores=no_cores, prop=TRUE, specenr=TRUE)
```
...
and grandparent cell populations (A+, B+, C+) are available. If not all of them 
are given, then the SpecEnr value calculated may not be accurate.
